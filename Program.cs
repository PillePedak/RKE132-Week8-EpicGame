...
string[] herose = File.ReadAllLines(Path.Combine(folderPath, heroFile));
string[] villains = File.ReadAllLines(Path.Combine(folderPath, villainFile));
string[] weapon = {"magic wand", "plastic fork", "banana", "wooden sword", "lego brick"};


string hero = GetRandomValueFromArray(herose);
...
{
    heroHP = heroHP - Hit(villain, villainStrikeStrengt);
...
{
    Console.WriteLine($"Dard side wins!");
}
else
{
...
static int Hit(string characterName, int characterHP)
{
    ...
    if(strike == 0)
    {
        Console.WriteLine($"{characterName} missed the targer!");
    }
